...
The central normalized data contract produced by all data adapters:

| Field Name | Type | Unit | Nullable | Description |
|---|---|---|---|---|
| `station_id` | `string` | — | No | Unique identifier for the Automatic Weather Station (e.g., `"AWS_IND_MH_012"`) |
| `timestamp` | `datetime (UTC)` | ISO-8601 | No | Observation recording timestamp in UTC |
| `temperature` | `float` | °C | Yes (if missing) | Ambient air temperature (physical bounds: $-50.0$ to $+60.0$) |
| `pressure` | `float` | hPa | Yes (if missing) | Atmospheric / barometric pressure (physical bounds: $500.0$ to $1080.0$) |
| `humidity` | `float` | % | Yes (if missing) | Relative humidity (physical bounds: $0.0$ to $100.0$) |
| `latitude` | `float` | Decimal degrees | No | Station latitude ($-90.0$ to $+90.0$) |
| `longitude` | `float` | Decimal degrees | No | Station longitude ($-180.0$ to $+180.0$) |
| `elevation` | `float` | Meters | Yes | Station elevation above sea level |
| `source` | `ObservationSource (Enum)` | — | No | Origin source: `HISTORICAL_CSV`, `SIMULATOR`, `WEATHER_API`, `MQTT` |
| `ingestion_timestamp` | `datetime (UTC)` | ISO-8601 | No | Server timestamp when the record entered the SkyGuard pipeline |
| `metadata` | `dict[str, Any]` | — | No | Additional sensor telemetry (battery voltage, signal strength, firmware) |
